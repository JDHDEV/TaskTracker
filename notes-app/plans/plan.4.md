...
### Track 4 (JIRA enrichment) — shipped 2026-07-17

Implemented (§6 steps 18–24), including the Critical SSRF item. New backend module `src-tauri/src/jira/mod.rs` = the third swap point (`JiraProvider` trait + `jira_provider_for`; Cloud impl `atlassian-cloud`), mirroring `AiProvider`/`ItemRepository`. `get_jira_ticket` re-extracts the ticket key **server-side** (`extract_ticket_key`, a hand-checked Rust port of `jira.ts`'s anchored `^[A-Za-z][A-Za-z0-9]*-\d+$` on the last path segment, parsed via `Url` so `?`/`#`/`../`/whitespace can't smuggle through) and builds the request destination **only** from the configured `JiraConfig.base_url` (`issue_url`) — the stored `jira_url`'s host is discarded after key extraction and is never a `reqwest` destination (K7). Failure ordering: a keyless/unparseable URL returns `Invalid` before any config/token/network work. Cloud auth is HTTP Basic `email:api_token`, `https`-only (`validate_base_url` enforced on save *and* before each fetch), 15s timeout, distinct 401/403/404/429 → clean status-only errors (no auth header, no body). New `AppError::{NotConfigured, RateLimited}`. Token in the keyring under `jira::TOKEN_ID = "atlassian"` (boolean-only `has_jira_token`, empty deletes, never returned); the non-secret `JiraConfig{baseUrl,email}` in a new additive `app_settings(key,value)` table via new `ItemRepository::{get_setting,set_setting}` (migration `0004`) — NOT the keyring (K6). `TicketMeta{key,title,status,statusCategory,fetchedAt}` + `JiraConfig` mirrored in `types.ts`. Frontend: `JiraChip.tsx` + `useJiraEnrichment(url)` hook (debounce 400ms, per-url session cache, states idle/unconfigured/loading/loaded/error); `JiraRow.tsx` delegates to the chip; the chip **always** opens the browser via `api.openExternal` regardless of state; loaded shows a status dot reusing `.dot-todo/-doing/-done` mapped from `statusCategory` (zero new colors) + the status word in text (WCAG 1.4.1) + ellipsized `.jira-chip-text`; title/status render as React text nodes only. `SettingsDialog.tsx` gained a JIRA connection + token block (heading + `App.tsx` button renamed `API keys` → `Settings`). New DESIGN.md "JIRA enrichment (new)" subsection + the Settings dialog spec updated.

**Deviations / decisions:**
1. **State set narrowed to match DESIGN.md, not the roadmap's wording.** §6 step 23 / §8 list `no-token/loading/loaded/stale/error` with "no config vs no token vs 401 vs 404 vs 429 vs network — each a distinct state". The shipped `EnrichmentState = idle | unconfigured | loading | loaded | error` collapses no-config/no-token into one quiet `unconfigured` label and folds every lookup failure into one quiet `error` fallback (the chip is a fallback, not an alarm), and does **not** implement a `stale`/TTL state. This follows DESIGN.md (authoritative per CLAUDE.md: "Where the HTML prototype disagrees, DESIGN.md wins") and the minimal-code rule — a compliant deviation, recorded here so the roadmap/shipped discrepancy doesn't read as an oversight. On-demand only, no background refresh.
2. **No new dependency.** The `jira` module reuses the shared `reqwest::Client` (`.basic_auth()`), `serde_json`, and `chrono`; URL parsing uses reqwest's re-exported `Url`; the key regex is hand-checked (no `regex` crate). `Cargo.toml` unchanged.
3. **`get_jira_ticket` isn't unit-testable** (needs `State<AppState>` + network, exactly like `ai_rewrite`/`ai_rewrite_stream`). The SSRF guard is therefore tested via the pure `extract_ticket_key` + `issue_url` composition (`resolve_destination` in `tests/jira.rs`), which faithfully mirrors the command's flow and was proven **red-then-green** against a "dial `jira_url`'s host" regression (temporarily hardcoding `issue_url` to a metadata host — the host-pin test failed, then passed on revert).

**Tests:** `src-tauri/tests/jira.rs` (19) — key extraction (browse URL, case-fold, trailing slash, query/fragment stripping, malformed/keyless), the two SSRF-guard tests (proven red-then-green), `validate_base_url` https-only, the full response-mapping matrix (all three status categories, missing summary/status, markup passthrough verbatim, unreadable body → clean error), and status classification (429→RateLimited, 401/403/404→Provider, auth message names no header). `tests/repo.rs` +1 (`app_settings` upsert/isolation round-trip). Full suite: **vitest 51, cargo test 97 (15 lib + 54 repo + 9 streaming + 19 jira), tsc/build clean.**

Review outcomes (security-auditor + code-reviewer, read-only on the diff):

1. **Security — clean; the Critical SSRF item SATISFIED, no High/Medium.** Host-pinning, https/TLS-on (no `danger_accept_invalid_certs`), keyring-only token attached only as a header and never in an error/URL/log, and React-text rendering with non-blocking degradation were all confirmed with file:line evidence.
2. **Redirect-policy hardening (auditor Low — fixed).** The shared `reqwest::Client` used the default redirect policy (follows up to 10), so a redirect returned by the trusted, TLS-verified configured host could still reach an internal address — a residual bypass of the host-pinning boundary (not directly exploitable: `base_url` is set only by the trusted local user, TLS is on, and reqwest strips the `Authorization` header on cross-origin redirects). Now built with `redirect::Policy::none()` in `lib.rs`. The client is shared with the AI providers; neither the Anthropic nor OpenAI endpoints legitimately redirect, so disabling globally is safe (streaming/rewrite suites stay green).
3. **Mid-session enrichment refresh (code-reviewer Warning — fixed).** The hook keyed its effect on the URL only, so configuring JIRA *while already viewing a linked item* (Settings is an overlay; the editor, keyed by `selected.id`, doesn't remount) left the chip on its plain label until the URL changed. Added a module-level epoch + subscriber in `useJiraEnrichment.ts` (`invalidateJiraEnrichment()`); `SettingsDialog` calls it after a successful connection/token save, so an on-screen chip re-enriches immediately. Fulfills the §9 "live" criterion for the first-run flow.
4. **Nits — left as-is (recorded).** Cache is keyed by the raw trimmed URL (two URLs resolving to the same ticket fetch twice) and never evicts; `set_jira_config` re-trims an already-trimmed email (intentional "never trust the frontend"). All negligible at single-user desktop scale; not worth added complexity.

**Manual smoke remaining (interactive session, cannot run here):** with a real Atlassian site URL + email + API token configured, open an item linked to a real ticket and confirm the chip enriches to `{KEY} — {title} · {status} ↗` with the correct status-dot color; that a title containing markup renders as literal text; that 401 (bad token) / 404 (unknown ticket) / no-config each degrade to the plain label without blocking the browser open; and — the security check — that outbound requests go **only** to the configured Atlassian host even when the stored `jira_url` points elsewhere.
