...
- Native context-menu items + `Insert timestamp` — **shipped** (plans/plan.16.md): the app no longer draws a right-click menu; Rust appends its items to WebView2's own menu (every default item, spell-check included, stays). Every body textarea (items, prompts, scratch) gets `Insert timestamp` (`yyyy-mm-dd HH:mm` local, at the caret, replacing any selection); the pad's "New note/task/prompt from selection" items ride the same native menu.
