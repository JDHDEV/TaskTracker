...
**About** — a small card on the scrim opened from the topbar `About` button: heading `worknotes`, a muted `Version {n}` line (read at runtime from the app metadata), the muted tagline `Local-first notes and tasks, refined with AI.`, a quiet `View on GitHub` button (opens the repository in the default browser via the guarded external-open path — never in-webview), and a right-aligned `Close`. Follows the Prompt-history discipline: initial focus on `Close`, Escape and scrim-click close it.
